...
public:
  ...
  X last_x_ = 0;
  Point global_origin_{0, 0};   // it must be the origin point of global slope line
  ...
  ConstraintGenerator(Y local_epsilon, Y global_epsilon)
  ...
  void generate_local_slope(Y target_rank) {
    ...
    X min_x_by_length = local_origin_.x + (target_rank - local_origin_.y); 
    // std::cout << "global min_x = " << min_x << std::endl;
    min_x = std::max(min_x, last_x_ + 1);
    min_x = std::max(min_x, min_x_by_length);
    ...
    // std::cout << "origin: " << local_origin_.x << ", " << local_origin_.y << std::endl;
    // std::cout << "new_xy: " << new_xy.x << ", " << new_xy.y << std::endl;
    // std::cout << "global slope = "
    //           << static_cast<long double>(global_slope_) << std::endl;
...
void dataGenerator(int local_epsilon, int global_epsilon, int local_value,
  ...
  std::random_device rd;
  std::mt19937 gen(rd());
  double global_expectation = (double)num / global_value;
  double global_sigma = 0.6;
  double global_miu = std::log(global_expectation) - std::pow(global_sigma, 2) / 2;
  std::lognormal_distribution<> global_seg(global_miu, global_sigma);
  std::cout << "global distribution: " << global_miu << ", " << global_sigma << std::endl;

  double local_expectation = (double)num / local_value;
  double local_sigma = 0.6;
  double local_miu = std::log(local_expectation) - std::pow(local_sigma, 2) / 2;
  std::lognormal_distribution<> local_seg(local_miu, local_sigma);
  std::cout << "local distribution: " << local_miu << ", " << local_sigma << std::endl;

  // generate global segment length
  std::vector<int> global_seg_len;
  int cur_key_num = 0;
  while (true) {
    int cur_global_seg_len = std::round(global_seg(gen));
    if (cur_key_num + cur_global_seg_len >= num) {
      break;
    }
    global_seg_len.push_back(cur_global_seg_len);
    cur_key_num += cur_global_seg_len;
  }
  int num_key_left = num - cur_key_num;
  if (num_key_left < 0.2 * global_expectation) {
    global_seg_len[global_seg_len.size() - 1] += num_key_left;
  } else {
    global_seg_len.push_back(num_key_left);
  }
  std::cout << "global seg num: " << global_seg_len.size() << ", " << global_seg_len[0] << std::endl;

  // generate local segment length
  std::vector<int> local_seg_len;
  std::vector<int> global_local_seg_num;
  for (int i = 0; i < global_seg_len.size(); i++) {
    int cur_global_seg_len = global_seg_len[i];
    int cur_seg_key_num = 0;
    int cur_seg_local_seg_num = 0;
    while (true) {
      int cur_local_seg_len = std::round(local_seg(gen));
      if (cur_seg_key_num + cur_local_seg_len >= cur_global_seg_len) {
        break;
      }
      local_seg_len.push_back(cur_local_seg_len);
      cur_seg_key_num += cur_local_seg_len;
      cur_seg_local_seg_num++;
    }
    int num_seg_key_left = cur_global_seg_len - cur_seg_key_num;
    if (num_seg_key_left < 0.2 * local_expectation) {
      local_seg_len[local_seg_len.size() - 1] += num_seg_key_left;
    } else {
      local_seg_len.push_back(num_seg_key_left);
      cur_seg_local_seg_num++;
    }
    global_local_seg_num.push_back(cur_seg_local_seg_num);
  }
  std::cout << "local seg num: " << local_seg_len.size() << ", " << local_seg_len[0] <<std::endl;

  int empty_in_100 = 10;
  int * pos_for_empty = new int[empty_in_100]();
  std::vector<int> empty_len;
  empty_len.push_back(empty_in_100);
  size_t cur_seed = gen();
  ScrambledZipfianGenerator zipf_gen(empty_in_100, &cur_seed);
  for (int i = 0; i < empty_in_100; i++) {
      int pos = zipf_gen.nextValue();
      pos_for_empty[pos]++;
  }
  for (int i = 0; i < empty_in_100; i++) {
    if (pos_for_empty[i]!= 0) {
      empty_len.push_back(pos_for_empty[i]);
    }
  }
  double avg_slope = 0.05;
  double total_key_range = num / avg_slope;
  double additional_key_range = total_key_range / 100.;
  int empty_num = (empty_len.size() == 0) ? 1 : empty_len.size();
  // int empty_interval = std::ceil((double)global_seg_len.size() / 2);
  int empty_interval = global_seg_len.size() / empty_num;
  // test empty segs
  // std::sort(empty_len.begin(), empty_len.end(), std::greater<int>());
  // int sum_empty = 0;
  // for (int i = 0; i < empty_len.size(); i++) {
  //   std::cout << empty_len[i] << std::endl;
  //   sum_empty += empty_len[i];
  // }
  ...
  // test seg(s) 
  // int seg_sum = 0;
  // int global_key_num = 0;
  // for (int i = 0; i < global_seg_len.size(); i++) {
  //   global_key_num+= global_seg_len[i];
  // }
  // std::cout << "global key num: " << global_key_num << std::endl;
  // int local_key_num = 0;
  // for (int i = 0; i < local_seg_len.size(); i++) {
  //   local_key_num+= local_seg_len[i];
  // }
  // std::cout << "local key num: " << local_key_num << std::endl;
  // int cur_l_seg_num = 0;
  // for (int i = 0; i < global_local_seg_num.size(); i++) {
  //   seg_sum += global_local_seg_num[i];
  //   int cur_g_seg_key_num = 0;
  //   for (int j = 0; j < global_local_seg_num[i]; j++) {
  //     cur_g_seg_key_num += local_seg_len[cur_l_seg_num];
  //     cur_l_seg_num++;
  //   }
  //   if (cur_g_seg_key_num != global_seg_len[i]) {
  //     std::cout << i << ": " << cur_g_seg_key_num << ", " << global_seg_len[i] << std::endl;
  //   }
  // }
  // std::cout << "global_local_seg_num size: " << global_local_seg_num.size() << ", " << seg_sum << std::endl;
  // exit(1);
  ...
  ConstraintGenerator<int64_t, int64_t> my_generator(
      local_epsilon, global_epsilon);
  my_generator.generate_global_slope();
  my_generator.generate_local_slope(local_seg_len[0]);
  ...
  int cur_local_num = 0;
  for (int i = 0; i < global_seg_len.size() - 1; ++i) {
    std::cout << "global seg " << i << std::endl;
    for (int j = 0; j < global_local_seg_num[i] - 1; ++j) {
      for (int k = 0; k < local_seg_len[cur_local_num] - 1; ++k) {
      ...
      // begin the next local segment
      if (j != global_local_seg_num[i] - 2) {
        cur_local_num++;
        cur_key = my_generator.generate_outside_local(
            cur_rank, cur_rank + local_seg_len[cur_local_num] - 1);
        data[cur_rank] = cur_key;
        if (!global_pla.check_point(cur_key, cur_rank)) {
          global_pla.reset();
        }
        ...
        // std::cout << "next local seg: " << local_seg_len[cur_local_num] << std::endl;
        // std::cout << "cur global seg: " << global_seg_len[i] << std::endl;
        // std::cout << j << " in " << global_local_seg_num[i] << std::endl;
        my_generator.generate_local_slope(cur_rank + local_seg_len[cur_local_num] - 1);
        cur_rank++;
      }
      // std::cout << "inside loop curr_rank = " << cur_rank << std::endl;
    }

    // begin the last local segment
    if (global_local_seg_num[i] != 1) {
      cur_local_num++;
      cur_key = my_generator.generate_outside_local(
          cur_rank, cur_rank + local_seg_len[cur_local_num] - 1);
      data[cur_rank] = cur_key;
      ...
      global_pla.add_point(cur_key, cur_rank);
      my_generator.set_local_origin(cur_key, cur_rank);
      // std::cout << "last local seg in global " << i << std::endl;
      my_generator.generate_local_slope(cur_rank + local_seg_len[cur_local_num] -
                                        1);
      cur_rank++;
    }
    ...
    // before the second last global segment, prepare for the next segment
    if (i != global_seg_len.size() - 2) {
      // std::cout << "begin the next global seg: " << cur_rank << std::endl;
      cur_key = my_generator.generate_outside_global(cur_rank);
      // std::cout << "cur_key outside_global: " << cur_key << std::endl;
      // std::cout << "cur points_in_hull: " << global_pla.get_points_in_hull() << std::endl;
      // std::cout << "keys in cur g seg: " << global_seg_len[i] << std::endl;
      int break_attempt = 0;
      while (global_pla.check_point(cur_key, cur_rank)) {
        if(break_attempt > 1000000) {
          cur_key += break_attempt;
          break;
        }
        ++cur_key;
        ++break_attempt;
      ...
      if (i % empty_interval == 0) {
        uint64_t cur_empty_range = empty_len[i/empty_interval] * additional_key_range;
        // uint64_t cur_empty_range = cur_key * 4;
        cur_key += cur_empty_range;
      }
      global_pla.reset();
      global_pla.add_point(cur_key, cur_rank);
      data[cur_rank] = cur_key;
      ...
      my_generator.generate_global_slope();
      cur_local_num++;
      // std::cout << "begin next global" << std::endl;
      my_generator.generate_local_slope(cur_rank + local_seg_len[cur_local_num] - 1);
      cur_rank++;
    }
  }
  ...
  if (global_seg_len.size() != 1) {
    ...
    // if ((global_seg_len.size()-1) % empty_interval == 0) {
    //   uint64_t cur_empty_range = empty_len[(global_seg_len.size()-1)/empty_interval] * additional_key_range;
    //   cur_key += cur_empty_range;
    // }
    global_pla.reset();
    global_pla.add_point(cur_key, cur_rank);
    ...
    my_generator.generate_global_slope();
    cur_local_num++;
    // std::cout << "fisrt local seg in last global seg" << std::endl;
    my_generator.generate_local_slope(cur_rank + local_seg_len[cur_local_num] - 1);
    cur_rank++;
  }
  int local_models_in_last_global_model = global_local_seg_num[global_local_seg_num.size() - 1];
  // std::cout << "local segs in last global seg: " << local_models_in_last_global_model << std::endl;
  for (int j = 0; j < local_models_in_last_global_model - 1; ++j) {
    for (int k = 0; k < local_seg_len[cur_local_num] - 1; ++k) {
      cur_key = my_generator.generate_key(cur_rank);
      data[cur_rank] = cur_key;
      cur_rank++;
    ...
    if (j != local_models_in_last_global_model - 2) {
      cur_local_num++;
      cur_key = my_generator.generate_outside_local(
          cur_rank, cur_rank + local_seg_len[cur_local_num] - 1);
      data[cur_rank] = cur_key;
      my_generator.set_local_origin(cur_key, cur_rank);
      // std::cout << j << " in " << local_models_in_last_global_model << std::endl;
      my_generator.generate_local_slope(cur_rank + local_seg_len[cur_local_num] - 1);
      cur_rank++;
    }
  ...
  if (local_models_in_last_global_model != 1) {
    cur_local_num++;
    cur_key = my_generator.generate_outside_local(
        cur_rank, cur_rank + local_seg_len[cur_local_num] - 1);
    data[cur_rank] = cur_key;
    my_generator.set_local_origin(cur_key, cur_rank);
    // std::cout << "begin last local" << std::endl;
    my_generator.generate_local_slope(cur_rank + local_seg_len[cur_local_num] - 1);
    cur_rank++;
  }

  // Handle last local segment
  // std::cout << "cur_local_num: " << cur_local_num << std::endl;
  for (int k = 0; k < local_seg_len[cur_local_num] - 1; ++k) {
    cur_key = my_generator.generate_key(cur_rank);
    data[cur_rank] = cur_key;
    cur_rank++;
