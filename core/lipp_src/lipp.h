...
public:
    ...
    bool is_leftmost_empty(Node* node, int pos) const {
        if (pos < 0) 
            return true;
        return (node->items[pos].comp.data.key == 0 && node->items[pos].comp.data.value == 0);
    } 
    P at(const T& key) const {
        ...
        while (true) {
            ...
            if (BITMAP_GET(node->none_bitmap, pos) == 1) {
                // empty
                if (is_leftmost_empty(node, pos)) {
                    // go up until not empty or root
                    go_up_num++;
                    auto tn = traversal_path.back();
                    traversal_path.pop_back();
                    Node* parent = tn.node;
                    int bucket_id = tn.bucketID;
                    while (!traversal_path.empty() && is_leftmost_empty(parent, bucket_id-1)) {
                        tn = traversal_path.back();
                        traversal_path.pop_back();
                        parent = tn.node;
                        bucket_id = tn.bucketID;
                    }
                    if (bucket_id-1 < 0) {
                        return static_cast<P>(0);
                    }
                    if (BITMAP_GET(parent->child_bitmap, bucket_id-1) == 1) {
                        node = parent->items[bucket_id-1].comp.child;
                        while (BITMAP_GET(node->child_bitmap, node->num_items-1) == 1) {
                            node = node->items[node->num_items-1].comp.child;
                        }
                        child_right_most_num++;
                        return node->items[node->num_items-1].comp.data.value;
                    }
                    return parent->items[bucket_id-1].comp.data.value;
                }
                // fake child
                if (BITMAP_GET(node->child_bitmap, pos) == 1) {
                    // return the rightmost key of this fake child
            ...
            // true child
            if (BITMAP_GET(node->child_bitmap, pos) == 1) {
                traversal_path.push_back({node, pos});
                node = node->items[pos].comp.child;
                cur_depth++;
                continue;
            // true value
                ...
                // validate value
                if (key >= node->items[pos].comp.data.key) {
                    return node->items[pos].comp.data.value;
                } 
                // left is empty
                if (is_leftmost_empty(node, pos-1)) {
                    if (pos-1 < 0) {
                        return static_cast<P>(0);
                    }
                    // go up
                ...
                // left is child
                if (BITMAP_GET(node->child_bitmap, pos-1) == 1) {
                    node = node->items[pos-1].comp.child;
                    while (BITMAP_GET(node->child_bitmap, node->num_items-1) == 1) {
                        node = node->items[node->num_items-1].comp.child;
                    }
                    child_right_most_num++;
                ...
                // left is value
                return node->items[pos-1].comp.data.value;
            }
        }
        return 0;
    }
    ...
    bool update(const T &key, const P& value) {
        for (Node* node = root; ; ) {
            ...
            if (BITMAP_GET(node->none_bitmap, pos) == 1) {
                ...
                for (int i =pos+1; i<node->num_items; i++) {
...
private:
    ...
    Node* build_tree_two(T key1, P value1, T key2, P value2)
    {
        ...
        // fill the empty items with next item
        int pos1 = PREDICT_POS(node, key1);
        int pos2 = PREDICT_POS(node, key2);
        for (int i = 0; i < pos1; i++) {
            node->items[i].comp.data.key = 0;
            node->items[i].comp.data.value = 0;
        }
        for (int i = pos1+1; i < pos2; i++) {
            node->items[i].comp.data.key = key1;
            node->items[i].comp.data.value = value1;
        }
        for (int i = pos2+1; i < node->num_items; i++) {
            node->items[i].comp.data.key = key2;
            node->items[i].comp.data.value = value2;
        }
    ...
    Node* build_tree_bulk_fmcd(T* _keys, P* _values, int _size)
    {
        ...
        while (!s.empty()) {
            ...
            if (end - begin == 2) {
                ...
                // fill the empty items with the last not null item
                Node * last_node_filled = nullptr;
                T last_key_filled = 0;
                P last_value_filled = 0;
                for (int i = 0; i < PREDICT_POS(node, keys[0]); i++) {
                    node->items[i].comp.data.key = 0;
                    node->items[i].comp.data.value = 0;
                }
                for (int i = PREDICT_POS(node, keys[0]); i < node->num_items; i++) {
                    if (BITMAP_GET(node->none_bitmap, i) == 0) {
                        if(BITMAP_GET(node->child_bitmap, i) == 0) {
                            last_key_filled = node->items[i].comp.data.key;
                            last_value_filled = node->items[i].comp.data.value;

                            last_node_filled = nullptr;
                        } else {
                            last_node_filled = node->items[i].comp.child;

                            last_key_filled = 0;
                            last_value_filled = 0;
                        }
                    } else {
                        if (last_node_filled == nullptr) { // last one is key value
                            node->items[i].comp.data.key = last_key_filled;
                            node->items[i].comp.data.value = last_value_filled;
                        } else {
                            BITMAP_SET(node->child_bitmap, i);
                            node->items[i].comp.child = last_node_filled;
                        }
                    }
                }
            }
    ...
    Node* insert_tree(Node* _node, const T& key, const P& value, bool* ok = nullptr)
    {
        ...
        for (Node* node = _node; ; ) {
            ...
            if (BITMAP_GET(node->none_bitmap, pos) == 1) {
                BITMAP_CLEAR(node->none_bitmap, pos);
                BITMAP_CLEAR(node->child_bitmap, pos);
                node->items[pos].comp.data.key = key;
                node->items[pos].comp.data.value = value;

                for (int i =pos+1; i<node->num_items; i++) {
                    if (BITMAP_GET(node->none_bitmap, i) == 1) {
                        BITMAP_CLEAR(node->child_bitmap, i);
                        node->items[i].comp.data.key = key;
                        node->items[i].comp.data.value = value;
                    } else {
                        break;
                    }
                ...
                auto new_tree =  build_tree_two(key, value, node->items[pos].comp.data.key, node->items[pos].comp.data.value);
                node->items[pos].comp.child = new_tree;
                insert_to_data = 1;

                for (int i =pos+1; i<node->num_items; i++) {
                    if (BITMAP_GET(node->none_bitmap, i) == 1) {
                        node->items[i].comp.child = new_tree;
                        BITMAP_SET(node->child_bitmap, i);
                    } else {
                        break;
                    }
        ...
        for (int i = 0; i < path_size; i ++) {
            ...
            if (need_rebuild) {
                ...
                if (i > 0) {
                    ...
                    for (int j =pos+1; j<path[i-1]->num_items; j++) {
                        if (BITMAP_GET(path[i-1]->none_bitmap, j) == 1) {
                            path[i-1]->items[j].comp.child = new_node;
                        } else {
                            break;
                        }
                    }
