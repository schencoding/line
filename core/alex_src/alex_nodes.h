...
#include "alex_base.h"
// extern unsigned long long prediction_deviation;
...
class AlexDataNode : public AlexNode<T, P> {
 public:
  ...
  static constexpr double kMinDensity_ = 0.05;  // density after expanding, also
  ...
  void print_leaf_array() {
    for(int i = 0; i < data_capacity_; i++) {
      std::cout << ALEX_DATA_NODE_KEY_AT(i) << ", ";
    }
    std::cout << std::endl;
  }
  
  int find_leaf_node(const T& key) {
    int predicted_pos = predict_position(key);
    int pos = exponential_search_upper_bound(predicted_pos, key) - 1;
    return pos;
  }
