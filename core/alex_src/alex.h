...
#define INNER_ALEX_SAFE_LOOKUP 0
...
class Alex {
 ...
 public:
  ...
  struct DerivedParams {
    ...
    int max_fanout = 1 << 17;  // assumes 8-byte pointers
...
// node's parent.
 ...
 public:
  ...
  inline bool get_leaf_node(const T& key, P *leaf_ptr) const {
    data_node_type *leaf = get_leaf(key);
    // while ((leaf->prev_leaf_) && (leaf->first_key() > key)) {
    while ((leaf->prev_leaf_) && (leaf->min_key_ > key)) {
      // std::cout << leaf->min_key_ << ", " << leaf->first_key() << std::endl;
      leaf = leaf->prev_leaf_;
    } 
    int idx = leaf->find_leaf_node(key);
    // std::cout << idx << std::endl;
    // std::cout << leaf->data_capacity_ << std::endl;
    if (idx >= 0) {
      *leaf_ptr = leaf->get_payload(idx);
      return true;
    } 
    return false;
  }
 ...
 public:
  ...
  bool update(const T &key, const P &payload) const {
      data_node_type *leaf = get_leaf(key);
      auto ret = leaf->update(key, payload);
      return ret;
  }
 ...
 public:
  ...
  void print_depth() {
    ...
    for (NodeIterator node_it = NodeIterator(this); !node_it.is_end();
      ...
      AlexNode<T, P>* cur = node_it.current();
      if (cur->is_leaf_) {
        // std::cout << leaf_num << ": " << cur->is_leaf_ << std::endl;
        // std::cout << (static_cast<data_node_type*>(cur)->num_keys_) << " keys " << std::endl;
        leaf_num++;
        if(cur->level_ > max_depth){
          max_depth = cur->level_;
        }
        sum_depth += (cur->level_) * (static_cast<data_node_type*>(cur)->num_keys_);
        num_keys += static_cast<data_node_type*>(cur)->num_keys_;
      }
    }
    double avg_depth = (double)sum_depth/(double)num_keys;
    std::cout << "max depth: " << max_depth << std::endl
              << "avg depth: " << avg_depth << std::endl
              << "num keys: " << num_keys << std::endl;
  }

  void print_dn_size() {
    std::vector<long long> dn_sizes;
    for (NodeIterator node_it = NodeIterator(this); !node_it.is_end();
         node_it.next()) {
      AlexNode<T, P>* cur = node_it.current();
      if (cur->is_leaf_) {
        dn_sizes.push_back(static_cast<data_node_type*>(cur)->data_size());
      }
    }
    std::sort(dn_sizes.begin(), dn_sizes.end());
    long long sizes_len = dn_sizes.size();
    long long min_dn_size = dn_sizes[0];
    long long max_dn_size = dn_sizes[sizes_len - 1];
    int odd = sizes_len % 2;
    long long mid_dn_size = odd ? dn_sizes[sizes_len/2] : (dn_sizes[sizes_len/2 - 1] + dn_sizes[sizes_len/2])/2;

    std::cout << "min DN size: " << min_dn_size << std::endl
              << "max DN size: " << max_dn_size << std::endl
              << "mid DN size: " << mid_dn_size << std::endl;
  }
