...
#define INNER_ALEX_SAFE_LOOKUP 1

namespace inneralexol {
...
public:
  ...
  bool get_leaf_node(const T &key, P *payload) const {
    EpochGuard guard;
    do {
      data_node_type *leaf = get_leaf(key);
      ...
      auto ret_flag = leaf->find_leaf_node(key, payload);
      if (ret_flag == true)
        return true; // ret_flag == true means no concurrency conlict occurs
    } while (true);
  }
